...
const OVERVIEW = [
  'Single PC, pass-and-play. The screen is a shared "game progress dashboard" showing hearts, shields, marks/curses, charges, streaks, etc.',
  'All randomness (coin flips, Chaos Gamble\'s roll) resolves instantly on-screen - no moderator or dice needed. The one exception is Akyros\'s Hidden Mark: tell a moderator privately which enemy to mark, since the dashboard is a single shared screen and can\'t hide it from everyone at once.',
  ...
  'Each player has 20 seconds to choose an action/target before it is auto-picked, to keep the match moving.',
];
...
const CHARACTER_RULES = {
  ...
  akyros: [
    { h: 'Hidden Mark (normal, no damage)', b: 'Secretly marks a target via a private moderator call - stays fully hidden until revealed. Marks never expire. Each enemy can only ever be marked ONCE per match - no re-marking, even after that mark is revealed or used. Akyros\'s marks are cleared entirely if he\'s KO\'d.' },
  ...
  boingo: [
    { h: 'Chaos Gamble (normal, every turn, any target)', b: 'Instant app-side roll, no RPS or moderator needed: 33% chance -3 hearts (win), 33% chance -1 heart (draw), 34% chance miss (0 damage).' },
    { h: 'Jester Ball (special, 1x, thrown at one target)', b: 'Doesn\'t resolve immediately - sits with the holder until their own next turn, when handling it becomes their action (drag onto Boingo = Return, drag onto another player = Pass; picking a normal action instead auto-explodes it as Take). Return heals Boingo +4. Take costs the holder -4 but doesn\'t use up their turn. Only one Pass allowed total. A 20s safety timer auto-resolves as Take if ignored.' },
  ],
  blade: [
...
export function renderRulesModal(container, onClose) {
  ...
  h2.textContent = 'How to Play — Soul Clash';
